...
export default class PayrollItem extends Component {
  ...
  handlePayroll(){
    ...
    let totalTaxes = Number((this.props.oasdiTaxes + this.state.fed + this.props.hiTaxes) / 26).toFixed(2);
    let totalBenefits = Number(this.state.benefitsData.Health_Insurance + this.state.benefitsData.Dental_Insurance).toFixed(2);
    let netPay = Number(this.props.salary / 26).toFixed(2) - Number((this.props.oasdiTaxes + this.state.fed + this.props.hiTaxes) / 26).toFixed(2) - totalBenefits;
    ...
    axios
    .post("http://localhost:3001/postPayroll", data, config);
    window.location.reload();
  }

  render(){
  //shorten paycheck date
  const paycheckShort = this.state.data.PaycheckDate ? this.state.data.PaycheckDate.substring(0,10) : ''
  return (
    <>
      <div className="salary-item">
        <img className="payroll-img" src={this.props.image} alt={this.props.name} />
        <div className="salary-item-desc">
          <h5>{this.props.name}</h5>
          Salary: ${this.props.salary} <br />
          Taxes: ${this.props.oasdiTaxes + this.props.hiTaxes + this.state.fed} <br />
          Earnings: ${this.props.salary - (this.props.oasdiTaxes + this.props.hiTaxes + this.state.fed)}
        </div>
            ...
            <tbody>
              <tr>
                ...
                <td><b>OASDI: </b>
                  ...
                  <b>Total:</b> {this.state.data.Taxes}</td>
                  <td>
                  <b>Health Insurance:</b> {this.state.benefitsData.Health_Insurance} <br />
                  <b>Dental Insurance: </b> {this.state.benefitsData.Dental_Insurance} <br />
                  <b>Total: </b> {Number(this.state.benefitsData.Health_Insurance + this.state.benefitsData.Dental_Insurance)}
                </td>
      ...
      {/* PAYROLL MODAL */}
      <Modal show={this.state.showPay} onHide={this.handleShowClose} centered>
        <Modal.Header closeButton>
            ...
            <tbody>
              <tr>
                ...
                */}
                ...
                <td>
                  {Number(Number(this.props.salary / 26).toFixed(2) -
                    Number((this.props.oasdiTaxes + this.state.fed + this.props.hiTaxes) / 26).toFixed(2) -
                    Number(Number(this.state.benefitsData.Health_Insurance + this.state.benefitsData.Dental_Insurance).toFixed(2))).toFixed(2)} 
                </td>
              </tr>
            </tbody>
          </Table>
